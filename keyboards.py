...
class Keyboards:
    ...
    @classmethod
    def movie_buttons(cls, movies, bot_username: str):
        username = (bot_username or '').replace('@', '')
        rows = []
        for movie in movies:
            size_text = cls.format_size(movie.get('file_size'))
            rows.append([
                InlineKeyboardButton(
                    f"🎬 {movie['title']} • {size_text}",
                    url=f'https://t.me/{username}?start=movie_{movie["id"]}',
                )
            ])
    ...
    @staticmethod
    def promo_buttons(channel_username: str, group_username: str):
        channel = (channel_username or '').replace('@', '')
        group = (group_username or '').replace('@', '')
        return InlineKeyboardMarkup([
            [InlineKeyboardButton('👥 Join Group', url=f'https://t.me/{group}')],
    ...
    def adult_confirmation():
        return InlineKeyboardMarkup([
            [InlineKeyboardButton('✅ I am 18 or above', callback_data='adult_yes')],
            [InlineKeyboardButton('❌ I am under 18', callback_data='adult_no')],
        ])

    @staticmethod
    def duplicate_confirmation(token: str):
        return InlineKeyboardMarkup([
            [InlineKeyboardButton('✅ Save Anyway', callback_data=f'dup_save_{token}')],
            [InlineKeyboardButton('❌ Cancel Upload', callback_data=f'dup_cancel_{token}')],
        ])
