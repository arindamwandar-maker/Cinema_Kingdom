...
class Config:
    ...
    SHORTENER_RESPONSE_FORMAT = os.getenv('SHORTENER_RESPONSE_FORMAT', 'json').strip().lower() or 'json'
    LOCAL_TIMEZONE = os.getenv('LOCAL_TIMEZONE', 'Asia/Kolkata').strip() or 'Asia/Kolkata'
    ...
    PROMO_INTERVAL = max(120, _safe_int(os.getenv('PROMO_INTERVAL'), 120))

    ADULT_KEYWORDS = (
        '18+', 'adult', 'xxx', 'porn', 'erotic', 'uncut adult', 'mature 18'
    )
