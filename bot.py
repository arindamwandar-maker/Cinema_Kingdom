...
class CinemaKingdomBot:
    ...
    def run(self):
        ...
        # Channel posts may not be handled like ordinary user commands, so these regex handlers are added too.
        app.add_handler(MessageHandler(
            filters.Chat(self.config.STORAGE_CHANNEL) & filters.TEXT & filters.Regex(r'^/delete(?:@\w+)?\s+\d+\s*$'),
            self.admin_handler.delete_movie,
        ))
        app.add_handler(MessageHandler(
            filters.Chat(self.config.STORAGE_CHANNEL) & filters.TEXT & filters.Regex(r'^/restore(?:@\w+)?\s+\d+\s*$'),
