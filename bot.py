...
class CinemaKingdomBot:
    def __init__(self):
        ...
        self.db.seed_admins(self.config.ADMIN_IDS, self.config.SUPER_ADMIN_ID)
        ...
        self.moderation_handler = ModerationHandler(self.db, self.config)

        self.auto_delete = AutoDeleteService(self.db)
        self.sheet_sync = SheetSyncService(self.db, self.config)
        self.promotion = PromotionService(self.config)
    ...
    def run(self):
        ...
        app.add_handler(CommandHandler('sendgroup', self.admin_handler.send_group))
        app.add_handler(CommandHandler('sendchannel', self.admin_handler.send_channel))
        app.add_handler(CommandHandler('sendboth', self.admin_handler.send_both))
        app.add_handler(CommandHandler('delete', self.admin_handler.delete_movie))
        app.add_handler(CommandHandler('restore', self.admin_handler.restore_movie))
        app.add_handler(CommandHandler('addadmin', self.admin_handler.add_admin))
        app.add_handler(CommandHandler('removeadmin', self.admin_handler.remove_admin))
        app.add_handler(CommandHandler('admins', self.admin_handler.list_admins))
        app.add_handler(CommandHandler('sheettest', self.admin_handler.sheet_test))

        app.add_handler(MessageHandler(
            filters.Chat(self.config.STORAGE_CHANNEL)
            & filters.TEXT
            & filters.Regex(r'^/delete(?:@\w+)?\s+\d+\s*$'),
            self.admin_handler.delete_movie,
        ))
        app.add_handler(MessageHandler(
            filters.Chat(self.config.STORAGE_CHANNEL)
            & filters.TEXT
            & filters.Regex(r'^/restore(?:@\w+)?\s+\d+\s*$'),
        ...
        # Moderation must run before ordinary search processing.
        app.add_handler(MessageHandler(
            filters.Chat(self.config.GROUP_ID),
        ...
        app.job_queue.run_repeating(self.auto_delete.cleanup, interval=30, first=10)
        ...
        app.job_queue.run_repeating(
            ...
            interval=self.config.PROMO_INTERVAL,
            first=30,
        )

        logging.info('Cinema Kingdom Bot Started | DB=%s', self.config.DB_PATH)
