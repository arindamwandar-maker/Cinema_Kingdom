...
class StartHandler:
    ...
    async def start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if context.args:
            ...
            # From group result: /start movie_123 -> show Download button only
            ...
            # After ShrinkEarn redirect: /start <secure-token> -> send file
    ...
    async def help(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        message = update.effective_message
        if message:
            await message.reply_text(text=self.config.HELP_MSG, reply_markup=Keyboards.main_menu())
    ...
    async def send_movie_message(self, update: Update, movie: dict):
        ...
        if not message:
            return
        title = html.escape(movie["title"])
        is_admin = bool(user and user.id in self.config.ADMIN_IDS)
        text = (
            f"🎬 <b>{title}</b>\n\n"
            "Your movie is ready.\n\n"
        ...
        await message.reply_text(
            text=text,
            parse_mode=ParseMode.HTML,
            reply_markup=Keyboards.movie_button(movie["id"], is_admin=is_admin),
        )
